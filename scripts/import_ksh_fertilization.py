...
DATA_DIR = BASE_DIR / "data" / "raw" / "ksh" / "fertilization"
FILE_PATH = DATA_DIR / "stadat-mez0093-19.1.2.25-en.csv"

METRIC_HEADERS = {
    "Land area treated with chemical fertilisers, ha": ("fertilized_area", "ha"),
    "Quantity of applied fertilisers per hectare, t/ha": ("fertilizer_quantity_per_ha", "t/ha")
}
...
def validate_records(records):
    ...
    print(len(counties), len(years), len(metrics), len(expected_metrics))
    if len(counties) != 19:
        raise ValueError(
...
def import_records(connection, records):
    with connection.cursor() as cursor:
        ...
        cursor.execute(
            """
            DELETE FROM raw.ksh_fertilizatution;
            """
        )

        cursor.executemany(
            """
            INSERT INTO raw.ksh_fertilization (
                metric,
                county_name,
                year,
                value,
...
def main():
    print("Importing records from KSH Fertilization...")
